...
from scripts.boundary_prediction import BOUNDARY_PRED_CHECKPOINT_DIR, BOUNDARY_PRED_CONFIG_DIR, BOUNDARY_PRED_LOG_DIR
from scripts import set_determinstic_mode
from nets.boundary_prediction import mstcn
...
class Trainer:
    def __init__(self, experiment, device):
        ...
        self.ce_loss = nn.BCELoss().cuda(self.device)
    ...
    def __init__(self, feat_files, labels, logits, boundary_region_pad=5):
        ...
        for video_logits in logits:
            ...
            for i in range(video_len - 1):
                ...
                curr_logit = video_logits[i]
                if curr_logit != next_logit:
