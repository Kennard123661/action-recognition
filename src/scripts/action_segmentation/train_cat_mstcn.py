...
from nets.action_seg import cat_mstcn
import data.breakfast as breakfast
from scripts.action_segmentation.create_submission import get_cls_results

CHECKPOINT_DIR = os.path.join(ACTION_SEG_CHECKPOINT_DIR, 'cat-mstcn')
LOG_DIR = os.path.join(ACTION_SEG_LOG_DIR, 'cat-mstcn')
CONFIG_DIR = os.path.join(ACTION_SEG_CONFIG_DIR, 'cat-mstcn')
SUBMISSION_DIR = os.path.join('/mnt/HGST6/cs5242-project/submissions/action-segmentation/cat-mstcn-temp')
NUM_WORKERS = 2
...
class Trainer:
    def __init__(self, experiment, device):
        ...
        self.model = cat_mstcn.MultiStageModel(num_stages=N_STAGES, num_layers=N_LAYERS, num_f_maps=N_FEATURE_MAPS,
