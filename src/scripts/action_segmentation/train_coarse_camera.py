...
CHECKPOINT_DIR = os.path.join(ACTION_SEG_CHECKPOINT_DIR, 'coarse-camera')
LOG_DIR = os.path.join(ACTION_SEG_LOG_DIR, 'coarse-camera')
CONFIG_DIR = os.path.join(ACTION_SEG_CONFIG_DIR, 'coarse-camera')
SUBMISSION_DIR = os.path.join('/mnt/HGST6/cs5242-project/submissions/action-segmentation/coarse-camera-temp')
NUM_WORKERS = 2

N_STAGES = 4
N_LAYERS = 10
N_FEATURE_MAPS = 64
IN_CHANNELS = 2048 + len(breakfast.COARSE_LABELS) + len(breakfast.CAMERAS)
...
class TrainDataset(tdata.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        camera_label = os.path.split(video_feat_file)[-1].split('.')[0].split('_')[1]
        camera_logit = breakfast.CAMERAS.index(camera_label)
        ...
        coarse_features = np.zeros(shape=[IN_CHANNELS, features.shape[1]], dtype=np.float32)
        coarse_features[:len(features), :] = features
        coarse_features[coarse_logit + len(features), :] = 1
        coarse_features[camera_logit + len(features) + len(breakfast.COARSE_LABELS), :] = 1
