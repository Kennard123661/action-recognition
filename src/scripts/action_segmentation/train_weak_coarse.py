...
CHECKPOINT_DIR = os.path.join(ACTION_SEG_CHECKPOINT_DIR, 'weakly-supervised-coarse')
LOG_DIR = os.path.join(ACTION_SEG_LOG_DIR, 'weakly-supervised-coarse')
CONFIG_DIR = os.path.join(ACTION_SEG_CONFIG_DIR, 'weakly-supervised-coarse')
SUBMISSION_DIR = os.path.join('/mnt/HGST6/cs5242-project/submissions/action-segmentation/weakly-supervised-coarse-temp')
NUM_WORKERS = 2
...
class TrainDataset(tdata.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        coarse_source_features = np.zeros(shape=[len(source_features) + len(breakfast.COARSE_LABELS),
