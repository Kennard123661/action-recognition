...
class Trainer:
    def __init__(self, experiment, device):
        ...
        self.prediction_model = mstcn.MultiStageModel(num_stages=N_STAGES, num_layers=N_LAYERS, num_f_maps=N_FEATURE_MAPS,
                                                      dim=IN_CHANNELS, num_classes=breakfast.N_MSTCN_CLASSES)
        self.prediction_model = self.prediction_model.cuda(self.device)
        ...
        if configs['optim'] == 'adam':
            self.optimizer = optim.Adam(self.prediction_model.parameters(), lr=self.lr)
        elif configs['optim'] == 'sgd':
            self.optimizer = optim.SGD(self.prediction_model.parameters(), lr=self.lr, momentum=configs['momentum'],
    ...
    def train_step(self, train_dataset):
        ...
        for feats, logits, masks in tqdm(dataloader):
            ...
            predictions = self.prediction_model(feats, masks)
            loss = None
    ...
    def _save_checkpoint(self, checkpoint_name='model'):
        ...
        save_dict = {
            'model': self.prediction_model.state_dict(),
    ...
    def _load_checkpoint(self, checkpoint_name='model'):
        ...
        if os.path.exists(checkpoint_file):
            ...
            self.prediction_model.load_state_dict(checkpoint['model'])
