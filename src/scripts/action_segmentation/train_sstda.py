...
CHECKPOINT_DIR = os.path.join(ACTION_SEG_CHECKPOINT_DIR, 'sstda')
LOG_DIR = os.path.join(ACTION_SEG_LOG_DIR, 'sstda')
CONFIG_DIR = os.path.join(ACTION_SEG_CONFIG_DIR, 'sstda')
NUM_WORKERS = 2
...
class Trainer:
    ...
    def train(self, train_data, test_data):
        ...
        train_dataset = TrainDataset(source_feat_files=train_segments, source_labels=train_labels,
                                     source_logits=train_logits, target_feat_files=test_segments,
                                     target_labels=test_labels, target_logits=test_logits,
                                     source_batch_size=self.train_batch_size)
    ...
    def train_step(self, train_dataset):
        ...
        self.model.train()
        losses = []
        for feats, logits, masks, is_source in tqdm(dataloader):
            print(feats.shape)
            print(logits.shape)
            print(masks.shape)
            print(is_source.shape)
            exit()
...
class TrainDataset(tdata.Dataset):
    def __init__(self, source_feat_files, source_labels, source_logits, target_feat_files, target_labels,
                 target_logits, source_batch_size):
        ...
        self.target_batch_size = max(1, self.source_batch_size * (len(self.source_feat_files) //
                                                                  len(self.target_feat_files)))
        self.n_iterations = np.ceil(len(self.source_feat_files) / self.source_batch_size)

    def __len__(self):
        return self.n_iterations

    def __getitem__(self, idx):
        source_start = idx * self.source_batch_size
        source_end = min(self.target_batch_size, (idx+1) * self.source_batch_size)
        source_feat_files = self.source_feat_files[source_start:source_end]
        source_logits = self.source_logits[source_start:source_end]

        target_feat_files, target_logits = [], []
        source_idxs = np.random.sample(len(self.target_feat_files), size=self.target_batch_size)
        for i in source_idxs:
            target_logits.append(self.target_feat_files[i])
            target_logits.append(self.target_logits[i])

        is_source = torch.ones(self.target_batch_size + self.source_batch_size)
        is_source[self.source_batch_size:] = 0

        combined_feat_files = source_feat_files.extend(target_feat_files)
        combined_logits = source_logits.extend(target_feat_files)

        all_features, all_logits, all_masks = [], [], []
        for i, feat_file in enumerate(combined_feat_files):
            features = np.load(feat_file)
            logits = combined_logits[i]
            assert features.shape[1] == len(logits)
            features = features[:, ::SAMPLE_RATE]
            logits = logits[:, ::SAMPLE_RATE]
        ...
        padded_features = torch.zeros(len(all_features), IN_CHANNELS, max_video_length, dtype=torch.float)
        padded_logits = torch.zeros(len(all_features), max_video_length, dtype=torch.long) * -100
        masks = torch.zeros(len(all_features), breakfast.N_MSTCN_CLASSES, max_video_length, dtype=torch.float)

        for i, feature in enumerate(all_features):
            video_len = feature.shape[1]
            padded_features[i, :, :video_len] = feature
            padded_logits[i, :video_len] = all_logits[i]
            masks[i, :, :video_len] = torch.ones(breakfast.N_MSTCN_CLASSES, video_len)
        return padded_features, padded_logits, masks, is_source
