...
CHECKPOINT_DIR = os.path.join(ACTION_SEG_CHECKPOINT_DIR, 'coarse-inputs-boundary-true')
LOG_DIR = os.path.join(ACTION_SEG_LOG_DIR, 'coarse-inputs-boundary-true')
CONFIG_DIR = os.path.join(ACTION_SEG_CONFIG_DIR, 'coarse-inputs-boundary-true')
SUBMISSION_DIR = os.path.join(ROOT_DIR, 'submissions/action-segmentation/coarse-input-boundary-true-temp')
NUM_WORKERS = 2
...
class TrainDataset(tdata.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(1, video_len-1):
            ...
            curr_logit = logits[i]
            next_logit = logits[i+1]
            if (prev_logit != curr_logit) or (curr_logit != next_logit):  # set boundary to 1 if different.
