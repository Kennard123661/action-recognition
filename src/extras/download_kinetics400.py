...
def main(split, trim, n_jobs):
    global TOTAL_VIDEOS

    if split == 'train':
        output_dir = kinetics.TRAIN_VIDEO_DIR
        input_csv = kinetics.TRAIN_VIDEO_CSV
    elif split == 'test':
        output_dir = kinetics.TEST_VIDEO_DIR
        input_csv = kinetics.TEST_VIDEO_CSV
    elif split == 'val':
        output_dir = kinetics.VAL_VIDEO_DIR
        input_csv = kinetics.VAL_VIDEOS_CSV
    else:
    ...
    # Creates folders where videos will be saved later. Also create 'tmp' directory for temporary files
    folders_names = links_df['label'].unique().tolist() + ['tmp']
    label_to_dir = create_file_structure(path=output_dir, folders_names=folders_names)

    TOTAL_VIDEOS = links_df.shape[0]
    # Download files by links from dataframe
    Parallel(n_jobs=n_jobs)(delayed(download_clip)(row, label_to_dir, trim, count)
...
if __name__ == '__main__':
    description = 'Script for downloading and trimming videos from Kinetics-400 dataset.'
    p = argparse.ArgumentParser(description=description)
    p.add_argument('--split', required=True, choices=['train', 'test', 'val'])
    ...
    p.add_argument('--n_jobs', type=int, default=1, help='Number of parallel processes for downloading and trimming.')
    args = p.parse_args()
    main(args.split, args.trim, args.n_jobs)
